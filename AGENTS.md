...
- AI 上下文窗口默认 256K tokens；管理员只能选择经过校验的 384K 或 1M 预设。这是本地历史预算，
  供应商 HTTP body 仍受独立的安全上限约束。请求过大时保留 system 前缀和完整的 assistant
  tool-call/tool-result 组，丢弃最旧历史并对单条工具输出做有界截断；413 必须立即失败且不得指数重试。
