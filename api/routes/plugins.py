...
from modules import (
    ...
    PluginResponse, PluginListResponse, PluginInstallRequest, InstalledPluginResponse,
    PluginCreate, Server,
...
async def create_plugin(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            ...
            detail="Only administrators can create plugins"
        )
...
async def upload_plugin(
    ...
    install_path: str = Form(default="addons/counterstrikesharp/plugins", description="Installation path"),
    config_required: bool = Form(default=False, description="Whether plugin requires configuration"),
    current_user: User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_db)
):
    """
    ...
    if not file.filename.endswith('.tar.gz'):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Plugin file must be a .tar.gz archive"
        )
...
async def install_plugin(
    ...
    existing = await InstalledPlugin.get_by_server_and_plugin(db, server_id, install_request.plugin_id)
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if plugin.dependencies:
        try:
            ...
            for dep_id in dependency_ids:
                dep_installed = await InstalledPlugin.get_by_server_and_plugin(db, server_id, dep_id)
    ...
    success = await _install_plugin_to_server(
        server,
        plugin,
        install_request.custom_download_url,
        install_request.config_data,
        db
    )
    ...
    if success:
        return JSONResponse(
            ...
            content={"success": True, "message": f"Plugin {plugin.display_name} installed successfully"}
        )
    else:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def uninstall_plugin(
    ...
    result = await db.execute(
        select(InstalledPlugin).where(
            InstalledPlugin.id == installed_plugin_id,
            InstalledPlugin.server_id == server_id
        )
    )
    ...
    if plugin:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(f"Could not connect to server {server_id} to remove plugin files: {msg}")
        except Exception as e:
...
async def _install_plugin_to_server(
    ...
    try:
        ...
        logger.info(f"Successfully installed plugin {plugin.name} v{plugin.version} on server {server.id}")
        return True
