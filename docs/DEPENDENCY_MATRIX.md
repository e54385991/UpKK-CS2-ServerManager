...
| SQLAlchemy / SQLModel | `>=2.0.52` / `>=0.0.39` | PostgreSQL 主路径，短事务 |
...
| Node.js | 24 LTS | npm lock + 静态资源 vendoring |
| 前端 | Alpine.js、Bootstrap、xterm 等 | `package-lock.json` 锁定，`npm ci` 验证 |

关键安全包当前下限为 `boto3>=1.43.80`、`cryptography>=50.0.1`。Dependabot 每周检查
uv、npm、Docker Compose 和 GitHub Actions；补丁/次版本合并分组，主版本单独 PR。
