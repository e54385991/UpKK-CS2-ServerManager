...
uv、npm（`frontend/`）、Docker Compose 和 GitHub Actions；补丁/次版本合并分组，主版本单独 PR。TypeScript 7 与 ESLint 10 在 typescript-eslint 支持前保持忽略；架构检查使用 `grimp>=3.16,<3.17` 与 `import-linter>=2.14,<2.15`。
