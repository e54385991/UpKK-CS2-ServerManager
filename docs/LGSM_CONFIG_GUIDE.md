...
1. **Max players**: Reduce `max_players` if server struggles
2. **SourceTV**: Disable `tv_enable` if not needed (uses extra resources)

## Best Practices

1. **RCON Password**: Always set a strong `rcon_password` for security
2. **Server Password**: Use `server_password` for private servers
3. **Client Port**: Let it auto-calculate (game_port + 1) unless you have specific needs
4. **Custom Parameters**: Test `additional_parameters` thoroughly before production use
