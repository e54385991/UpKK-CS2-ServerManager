...
export async function GET(
  request: NextRequest,
  context: {
    params: Promise<{ initializedServerId: string; operationId: string }>;
  },
) {
  const token = sessionTokenFrom(await cookies());
  if (!token) return new Response("Authentication required", { status: 401 });

  const { initializedServerId, operationId } = await context.params;
  const after = request.nextUrl.searchParams.get("after") ?? "0";
  const upstream = await fetch(
    `${internalApiUrl()}/api/v1/setup/initialized-servers/${initializedServerId}/operations/${operationId}/events?after=${encodeURIComponent(after)}`,
    {
      headers: {
