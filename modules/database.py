...
async def migrate_db():
    ...
    async with engine.begin() as conn:
        ...
        # Add the system-wide GitHub fallback token without exposing or copying
        # any user's personal credential.
        result = await conn.execute(
            text("""
                SELECT COLUMN_NAME
                FROM INFORMATION_SCHEMA.COLUMNS
        ...
        # Keep existing administrators' explicit proxy selections intact while
        # making panel proxy the default for newly created settings records.
        result = await conn.execute(
            text("""
