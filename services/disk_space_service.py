...
class DiskSpaceService:
    ...
    async def get_disk_space(
        ...
        if success and disk_info:
            ...
            logger.info(
                f"Cached disk space for server {server.id}: {disk_info.get('used_gb', 0):.2f}GB used of {disk_info.get('total_gb', 0):.2f}GB"
            )
    ...
    async def _read_disk_space(self, server: Server) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            if not success:
                logger.warning(
                    f"Failed to connect to server {server.id} for disk space read: {msg}"
                )
                return False, None
