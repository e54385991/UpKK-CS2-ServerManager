...
_LOW_VIOLENCE_RE = re.compile(r"^\s*Game_LowViolence\b")
_ADDON_GAME_RE = re.compile(
    r"^\s*Game\s+(?P<path>csgo/addons/(?:metamod|swiftlys2))\s*(?://.*)?\s*$"
)
...
def rewrite_gameinfo_search_paths(
    ...
    *,
    ...
    insertions = []
    if need_metamod:
        insertions.append(f"{indent}Game\t{GAMEINFO_METAMOD_PATH}{newline}")
    if need_swiftly:
        insertions.append(f"{indent}Game\t{GAMEINFO_SWIFTLY_PATH}{newline}")
