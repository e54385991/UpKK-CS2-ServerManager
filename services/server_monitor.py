...
class ServerMonitor:
    ...
    async def monitor_server(self, server_id: int, ssh_manager, progress_callback=None):
        ...
        from modules.models import Server, ServerStatus, MonitoringLog
        ...
        try:
            while True:
                ...
                # Re-fetch server data for status check
                async with async_session_maker() as db:
                    result = await db.execute(select(Server).filter(Server.id == server_id))
                    server = result.scalar_one_or_none()
                    ...
                    if is_down and server.auto_restart_on_crash:
                        ...
                        if can_restart:
                            ...
                            # Log restart attempt with specific trigger
                            restart_log = MonitoringLog(
                                server_id=server_id,
                            ...
                            try:
                                ...
                                # Log restart exception
                                error_log = MonitoringLog(
                                    server_id=server_id,
                                    event_type='auto_restart',
                                    status='failed',
