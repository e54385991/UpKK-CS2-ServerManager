...
def test_ai_models_capabilities_and_path_boundaries():
    ...
    with pytest.raises(ValueError):
        module.FilePatchInput(
            relative_path="x", expected_revision="A" * 64, content="x"
        )
...
async def test_saved_commands_server_listing_and_runtime_profile(monkeypatch):
    ...
    monkeypatch.setattr(module.CustomCommand, "get_by_id_server_and_user", AsyncMock(return_value=command))
    monkeypatch.setattr(custom, "execute_custom_commands", AsyncMock(return_value={"success": True, "message": "ok"}))
    monkeypatch.setattr(module.maintenance_lock_service, "get", lambda *_a, **_k: _Lock())
    result = await module.execute_saved_host_command(
        ctx,
        module.SavedHostCommandInput(command_id=4, expected_command_hash=module._saved_command_hash(command)),
    )
    ...
    command.target = "host"
    monkeypatch.setattr(module.CustomCommand, "get_by_id_server_and_user", AsyncMock(return_value=None))
    with pytest.raises(ValueError, match="unavailable"):
        await module.execute_saved_host_command(
            ctx,
    ...
    monkeypatch.setattr(policy, "get_effective_agent_policy", AsyncMock(return_value=SimpleNamespace(enabled=True)))
    result = await module.list_servers(_ctx(None, db), module.EmptyInput())
    assert result["servers"][0]["name"] == "Demo"
    monkeypatch.setattr(policy, "get_effective_agent_policy", AsyncMock(return_value=SimpleNamespace(enabled=False)))
    ...
    monkeypatch.setattr(module, "authorized_server", AsyncMock(return_value=server))
    monkeypatch.setattr(runtime, "detect_linux_runtime_profile", AsyncMock(return_value={"reason": "ok"}))
...
async def test_market_preview_and_installed_inventory_variants(monkeypatch):
    ...
    monkeypatch.setattr(module.MarketPlugin, "get_by_ids", AsyncMock(return_value=[framework, ordinary]))
    conflict = importlib.import_module("services.plugin_conflict_service")
    monkeypatch.setattr(conflict, "_panel_framework_key", lambda plugin: "metamod" if plugin.id == 1 else None)
    monkeypatch.setattr(conflict, "_latest_release_asset", AsyncMock(return_value={"release_id": "r", "release_tag": "v1", "asset_name": "p-steamrt3.zip"}))
    runtime = importlib.import_module("services.linux_runtime_service")
    monkeypatch.setattr(runtime, "steam_runtime_for_asset", lambda name: "steamrt3" if "steamrt3" in name else None)
    ...
    tracked = [SimpleNamespace(id=2, display_name="Plugin", source_type="market", market_plugin_id=2, framework_key=None, installed_version="v1")]
    ctx = _ctx(server, _Db(tracked))
    monkeypatch.setattr(module, "authorized_server", AsyncMock(return_value=server))
    inventory = importlib.import_module("services.plugin_inventory_service")
    monkeypatch.setattr(module, "inspect_remote_plugin_inventory", AsyncMock(return_value={"frameworks": {}, "plugins": ["Plugin"], "truncated": False}))
    monkeypatch.setattr(module, "installation_evidence", lambda plugin, _inv: {"name": plugin.display_name})
    result = await module.list_installed_plugins(ctx, module.EmptyInput())
    assert result["remote_inspection"]["status"] == "success"
    monkeypatch.setattr(module, "inspect_remote_plugin_inventory", AsyncMock(side_effect=inventory.PluginInventoryError("offline")))
    result = await module.list_installed_plugins(ctx, module.EmptyInput())
...
async def test_ai_plan_wrappers_and_dispatch(monkeypatch):
    ...
    assert (await module.plan_workshop_map(ctx, module.WorkshopPlanInput(workshop_id_or_url="123")))["ok"]
    ...
    assert (await module.plan_server_startup_update(ctx, module.ServerStartupPlanInput(default_map="de_dust2")))["ok"]
    ...
    assert (await module.get_plugin_crash_isolation(ctx, module.DiagnosticRunInput(diagnostic_id="x" * 36)))["id"] == "x"
    monkeypatch.setattr(module, "TOOLS_BY_NAME", {"read": module.ToolSpec("read", "", "read", module.EmptyInput, AsyncMock(return_value={"ok": True}), requires_server=False)})
    policy = importlib.import_module("services.agent_policy_service")
...
async def test_ai_plugin_github_diagnostic_and_upgrade_wrappers(monkeypatch):
    ...
    monkeypatch.setattr(conflicts, "build_plugin_install_plan", AsyncMock(return_value={"installation_order": [4], "already_installed": []}))
    runtime = importlib.import_module("services.linux_runtime_service")
    monkeypatch.setattr(runtime, "detect_linux_runtime_profile", AsyncMock(return_value={"reason": "x"}))
    monkeypatch.setattr(module, "_market_release_selection_preview", AsyncMock(return_value=[{"plugin_id": 4}]))
    assert (await module.plan_plugin_install(ctx, module.PluginPlanInput(plugin_id=4)))["release_selections"]
    ...
    diagnostics = importlib.import_module("services.plugin_diagnostic_service")
    monkeypatch.setattr(diagnostics, "execute_diagnostic_plan", AsyncMock(return_value={"status": "done"}))
    assert (await module.execute_plugin_crash_isolation(ctx, module.DiagnosticExecuteInput(expected_plan_hash="a" * 64)))["status"] == "done"
    monkeypatch.setattr(diagnostics, "restore_diagnostic_run", AsyncMock(return_value={"status": "restored"}))
    assert (await module.restore_plugin_quarantine(ctx, module.DiagnosticRunInput(diagnostic_id="a" * 36)))["status"] == "restored"
    ...
    assert (await module.search_github_cs2_plugins(ctx, module.GitHubSearchInput(query="demo")))["candidates"] == []
    assert (await module.inspect_github_plugin(ctx, module.GitHubInspectInput(repo_url="https://github.com/acme/demo")))["repo_url"] == "x"
    monkeypatch.setattr(github, "build_github_install_plan", AsyncMock(return_value={"plan": True}))
    assert (await module.plan_github_plugin_install(ctx, module.GitHubPlanInput(repo_url="https://github.com/acme/demo")))["plan"]
    monkeypatch.setattr(github, "execute_github_install_plan", AsyncMock(return_value={"success": True}))
    assert (await module.apply_github_plugin_install(ctx, module.GitHubApplyInput(repo_url="https://github.com/acme/demo", expected_plan_hash="a" * 64)))["success"]

    monkeypatch.setattr(conflicts, "execute_plugin_install_plan", AsyncMock(return_value={"success": True}))
    ctx.enforce_agent_policy = False
    assert (await module.apply_plugin_plan(ctx, module.ApplyPluginPlanInput(plugin_id=4, expected_plan_hash="a" * 64)))["success"]
    startup = importlib.import_module("services.server_startup_service")
    monkeypatch.setattr(startup, "execute_server_startup_plan", AsyncMock(return_value={"success": True}))
    assert (await module.apply_server_startup_update(ctx, module.ApplyServerStartupPlanInput(default_map="de_dust2", expected_plan_hash="a" * 64)))["success"]

    singleton = importlib.import_module("services.plugin_auto_update_service").plugin_auto_update_service
    upgrade_plan = {"plugin_id": 4, "no_op": True}
    monkeypatch.setattr(singleton, "build_plugin_upgrade_plan", AsyncMock(return_value=upgrade_plan))
    no_op_hash = module.canonical_arguments(upgrade_plan)[1]
    assert (await module.apply_managed_plugin_upgrade(ctx, module.ApplyManagedPluginUpgradeInput(plugin_id=4, expected_plan_hash=no_op_hash)))["no_op"]
    ...
    monkeypatch.setattr(enqueue_module, "enqueue_plugin_auto_update", AsyncMock(return_value={"operation_id": "op"}))
    monkeypatch.setattr(hub.server_operation_hub, "wait_until_terminal", AsyncMock(return_value={"success": True, "message": "updated"}))
    expected = module.canonical_arguments(upgrade_plan)[1]
    result = await module.apply_managed_plugin_upgrade(ctx, module.ApplyManagedPluginUpgradeInput(plugin_id=4, expected_plan_hash=expected))
    assert result["success"]
