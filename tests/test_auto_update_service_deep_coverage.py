...
async def test_version_resolution_and_deadlines(monkeypatch):
    ...
    assert (await service._resolve_observed_version(server, "old", "1234", None, {}, progress))[0] is False
    ...
    assert not (await service._resolve_observed_version(server, "old4", None, deadline, {}, progress))[0]
    expired = asyncio.get_running_loop().time() - 1
    assert await service._read_version_with_deadline(server, expired) == (False, None)
    monkeypatch.setattr(auto_update.steam_inf_service, "refresh_version_cache", AsyncMock(return_value=(True, "v")))
...
async def test_wait_for_updated_version_all_terminal_paths(monkeypatch):
    ...
    monkeypatch.setattr(service, "_read_version_with_deadline", AsyncMock(return_value=(False, None)))
    assert await service._wait_for_updated_version(server, "target", progress) == (False, None, None)
    service.VERSION_VERIFICATION_TIMEOUT_SECONDS = 1
    service.VERSION_VERIFICATION_POLL_INTERVAL_SECONDS = 1
    monkeypatch.setattr(service, "_read_version_with_deadline", AsyncMock(side_effect=asyncio.TimeoutError))
    ...
    monkeypatch.setattr(service, "_read_version_with_deadline", AsyncMock(return_value=(True, "old")))
    monkeypatch.setattr(auto_update.steam_api_service, "check_version", AsyncMock(return_value=(False, None)))
    assert (await service._wait_for_updated_version(server, "target", progress))[0] is False
...
async def test_check_and_update_servers_filters_and_single_checks(monkeypatch):
    ...
    monkeypatch.setattr("modules.models.Server.get_all_with_auto_update", AsyncMock(return_value=servers))
    monkeypatch.setattr(auto_update.steam_api_service, "should_check_version", lambda last, interval: last != "recent")
    check = AsyncMock()
    ...
    db = _Db()
    monkeypatch.setattr("modules.database.async_session_maker", lambda: db)
    monkeypatch.setattr(auto_update.steam_inf_service, "get_version_from_steam_inf", AsyncMock(return_value=(False, None)))
    monkeypatch.setattr(auto_update.steam_api_service, "check_version", AsyncMock(return_value=(True, {"up_to_date": True})))
    ...
    await service._check_and_update_server(server)
    monkeypatch.setattr(auto_update.steam_inf_service, "get_version_from_steam_inf", AsyncMock(return_value=(True, "old")))
    monkeypatch.setattr(auto_update.steam_api_service, "check_version", AsyncMock(return_value=(True, {"up_to_date": False, "required_version": "new"})))
    await service._check_and_update_server(server)
...
async def test_update_guards_wait_cache_and_loop(monkeypatch):
    service = auto_update.AutoUpdateService()
    server = _server()
    monkeypatch.setattr("services.plugin_diagnostic_service.has_diagnostic_blocker", AsyncMock(return_value=True))
    assert await service._can_trigger_update(server) is False
    monkeypatch.setattr("services.plugin_diagnostic_service.has_diagnostic_blocker", AsyncMock(return_value=False))
    monkeypatch.setattr(auto_update.maintenance_lock_service, "is_locked", AsyncMock(return_value=True))
    assert await service._can_trigger_update(server) is False
    monkeypatch.setattr(auto_update.maintenance_lock_service, "is_locked", AsyncMock(return_value=False))
    assert await service._can_trigger_update(server) is True

    service.running = True
    monkeypatch.setattr(service, "_check_and_update_servers", AsyncMock(side_effect=RuntimeError("check")))
