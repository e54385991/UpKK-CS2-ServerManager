...
async def test_menu_component_identity_launcher_paging_and_action_paths(monkeypatch):
    ...
    await manager._handle_menu_component(
        client, interaction, ["cs2", "menu", "page", "1", "30", "2"]
    )
...
async def test_managed_plugin_map_and_market_component_paths(monkeypatch):
    ...
    monkeypatch.setattr(
        manager, "_resolve_menu_server", AsyncMock(return_value=SimpleNamespace(id=3, name="demo"))
    )
    managed = SimpleNamespace(
        id=7, server_id=3, display_name="Managed", installed_version="1", latest_version="2"
    )
    ...
    redis = SimpleNamespace(
        get=AsyncMock(
            return_value={
                ...
                "query": "dust",
                "matches": [
                    {"name": candidate.name, "filename": candidate.filename, "enabled": True}
                ],
            }
        ),
        delete=AsyncMock(),
...
async def test_menu_submit_helpers_and_ai_component_progress(monkeypatch):
    ...
    redis = SimpleNamespace(
        ...
        get=AsyncMock(return_value={"server_id": 3, "query": "x", "mode": "browse"}),
        delete=AsyncMock(),
    )
    monkeypatch.setattr(module, "redis_manager", redis)
    monkeypatch.setattr(manager, "_market_search_view", AsyncMock(return_value="view"))
    monkeypatch.setattr(module, "_edit_interaction_message", AsyncMock())
    await manager._menu_search_submit(
        client, interaction, issued_at=1, server_id=3, mode="browse", query=" x "
    )
    with pytest.raises(module.DiscordAuthorizationDenied):
        await manager._menu_search_submit(
            client, interaction, issued_at=1, server_id=3, mode="bad", query="x"
        )
    ...
    monkeypatch.setattr(
        module, "discord_run_snapshot", AsyncMock(return_value={"progress": {"status": "done"}})
    )
