...
def _server(**overrides):
    values = dict(
        id=3,
        user_id=7,
        enable_plugin_auto_update=False,
...
async def test_auto_update_helpers_and_configuration(monkeypatch):
    ...
    assert await routes.validate_post_update_commands(commands_db, server, user, [1, 2, 1]) == [
        1,
        2,
    ]
    with pytest.raises(HTTPException, match="Unknown"):
        await routes.validate_post_update_commands(
            _Db([_Rows([SimpleNamespace(id=1)])]), server, user, [1, 2]
        )

    config = await routes.get_configuration(3, _Db([_Rows([plugin])]), user)
    assert config.plugins[0].id == 4

    request = PluginAutoUpdateSettings(
        enable_plugin_auto_update=True,
        plugin_update_check_interval_hours=2,
        enable_plugin_post_update_commands=True,
        plugin_post_update_command_ids=[1],
    )
...
async def test_register_update_unmanage_and_run_routes(monkeypatch):
    ...
    with pytest.raises(HTTPException):
        await routes.register_plugin(
            3,
            ManagedPluginCreate(source_type="framework", framework_key="unknown", display_name="x"),
            _Db(),
            user,
        )
    with pytest.raises(HTTPException):
        await routes.register_plugin(
            3,
            ManagedPluginCreate(source_type="market", market_plugin_id=99, display_name="x"),
            _Db(get_value=None),
            user,
        )
    market = SimpleNamespace(
        id=99, github_url="https://github.com/acme/market", title="Market", custom_install_path=None
    )
    with pytest.raises(HTTPException):
        await routes.register_plugin(
            3,
            ManagedPluginCreate(source_type="market", market_plugin_id=99, display_name="x"),
            _Db(get_value=market),
            user,
        )
    ...
    updated = await routes.update_plugin(
        3, 4, ManagedPluginUpdate(auto_update_enabled=False), _Db(), user
    )
    ...
    monkeypatch.setattr(
        routes, "enqueue_plugin_auto_update", AsyncMock(side_effect=ServerOperationConflict("busy"))
    )
    with pytest.raises(HTTPException) as exc:
    ...
    monkeypatch.setattr(
        routes.plugin_auto_update_service, "get_status", AsyncMock(return_value={"running": False})
    )
