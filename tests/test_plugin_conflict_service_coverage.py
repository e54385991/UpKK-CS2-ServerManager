...
def test_conflict_helpers_and_progress_signatures():
    ...
    assert module._panel_framework_key(_plugin(url="https://github.com/alliedmodders/metamod-source")) == "metamod"
    assert module._panel_framework_key(_plugin(url="git@github.com:roflmuffin/counterstrikesharp.git")) == "counterstrikesharp"
    ...
    payload = module._plugin_plan_confirmation_payload(
        {"server_id": 1, "plugin": {}, "dependencies": [], "installation_order": [], "hard_conflicts": [], "warnings": [], "blocked": False, "extra": 1}
    )
    assert "extra" not in payload
    assert module._asset_from_download_url(_plugin(), "https://github.com/a/b/releases/download/v2/a.zip")["release_tag"] == "v2"
    custom = _plugin()
...
async def test_dependency_resolution_cycle_missing_and_build_plan(monkeypatch):
    ...
    monkeypatch.setattr(module.MarketPlugin, "get_by_id", AsyncMock(side_effect=lambda _db, key: plugins.get(key)))
    ...
    monkeypatch.setattr(module.MarketPlugin, "get_by_id", AsyncMock(side_effect=lambda _db, key: plugins.get(key)))
    monkeypatch.setattr(module, "inspect_remote_plugin_inventory", AsyncMock(return_value={"plugins": [], "frameworks": {}, "truncated": False}))
...
async def test_release_assets_latest_and_install_one_paths(monkeypatch):
    ...
    monkeypatch.setattr(module.http_helper, "get", AsyncMock(return_value=(True, {"id": 1, "tag_name": "v1", "assets": assets}, None)))
    ...
    github = __import__("services.github_plugin_plan_service", fromlist=["inspect_release_asset_layout"])
    monkeypatch.setattr(github, "inspect_release_asset_layout", AsyncMock(return_value={"mapping": [{"source": "addons", "target": "addons"}], "mapping_required": False, "archive_sha256": "a" * 64, "source_prefix": "addons"}))
    latest = await module._latest_release_asset(_Db(), plugin, _server(), SimpleNamespace(id=3), {"reason": "x"})
    ...
    monkeypatch.setattr(module, "_latest_release_asset", AsyncMock(return_value={"download_url": "https://github.com/a/b/releases/download/v1/p.zip", "release_id": "r", "release_tag": "v1", "asset_name": "p.zip", "custom_install_path": None, "source_prefix": None, "allowed_roots": ["addons"], "steam_runtime": None, "archive_sha256": None}))
    monkeypatch.setattr(module, "install_github_plugin", AsyncMock(return_value=GitHubPluginInstallResponse(success=True, message="ok", installed_files=1)))
    monkeypatch.setattr(module, "upsert_managed_plugin", AsyncMock())
    result = await module._install_one(_Db(), plugin, _server(), SimpleNamespace(id=3), exclude_files=["cfg/x"], upgrade_mode=True)
    assert result["success"] and plugin.install_count == 1
    monkeypatch.setattr(module, "install_github_plugin", AsyncMock(return_value=GitHubPluginInstallResponse(success=False, message="server not found")))
    failed = await module._install_one(_Db(), plugin, _server(), SimpleNamespace(id=3))
    assert not failed["success"]

    runtime_selection = __import__("services.linux_runtime_service", fromlist=["RuntimeSelectionRequired"])
    monkeypatch.setattr(module.http_helper, "get", AsyncMock(return_value=(True, {"id": 1, "tag_name": "v1", "assets": [{"name": "p-linux.zip", "browser_download_url": "https://github.com/a/b/releases/download/v1/p.zip"}, {"name": "p-linux2.zip", "browser_download_url": "https://github.com/a/b/releases/download/v1/p2.zip"}]}, None)))
    ...
    monkeypatch.setattr(module, "_latest_release_asset", AsyncMock(return_value={"download_url": "https://github.com/a/b/releases/download/v1/p.zip", "release_id": "r", "release_tag": "v1", "asset_name": "p.zip", "custom_install_path": None, "source_prefix": None, "allowed_roots": ["addons"], "archive_sha256": None, "steam_runtime": None}))
    install = AsyncMock(side_effect=[GitHubPluginInstallResponse(success=False, message="network timeout"), GitHubPluginInstallResponse(success=True, message="ok")])
    monkeypatch.setattr(module, "install_github_plugin", install)
    retry = await module._install_one(_Db(), plugin, _server(), SimpleNamespace(id=3), progress=AsyncMock())
...
async def test_panel_framework_and_execute_plan_paths(monkeypatch):
    ...
    db = _Db()
    server = _server()
    auto = __import__("services.plugin_auto_update_service", fromlist=["record_framework_installation"])
    monkeypatch.setattr(auto, "record_framework_installation", AsyncMock(side_effect=[RuntimeError("track"), None]))
    ssh_module = __import__("services.ssh_manager", fromlist=["SSHManager"])
    ssh = SimpleNamespace(install_metamod=AsyncMock(return_value=(True, "installed")), install_counterstrikesharp=AsyncMock(return_value=(False, "failed")))
    monkeypatch.setattr(ssh_module, "SSHManager", lambda: ssh)
    result = await module._install_panel_framework(db, plugin, server, SimpleNamespace(id=3), "metamod", None)
    assert result["success"] and "tracking_warning" in result
    counter = _plugin(2, title="CounterStrikeSharp")
    result = await module._install_panel_framework(db, counter, server, SimpleNamespace(id=3), "counterstrikesharp", None)
    assert not result["success"]

    plan = {"plan_hash": "h" * 64, "installation_order": [1], "already_installed": [1], "hard_conflicts": [], "warnings": [], "blocked": False, "dependencies": [], "plugin": {"id": 1, "title": "Plugin"}, "server_id": 7}
    target = _plugin(1)
    ...
    monkeypatch.setattr(module, "_prepare_plugin_execution", AsyncMock(return_value=(plan, {1}, {1: target}, {}, None)))
    success = await module.execute_plugin_install_plan(
        db, server, SimpleNamespace(id=3, is_admin=False), 1, expected_plan_hash="h" * 64, acquire_lock=False
    )
    assert success["success"] and success["completed"][0]["skipped"]

    plan["installation_order"] = [1, 2]
    second = _plugin(2)
    monkeypatch.setattr(module, "_prepare_plugin_execution", AsyncMock(return_value=(plan, set(), {1: target, 2: second}, {}, None)))
    monkeypatch.setattr(module, "_install_one", AsyncMock(return_value={"plugin_id": 1, "success": False, "message": "failed"}))
...
async def test_prepare_execution_and_lock_progress_error_paths(monkeypatch):
    ...
    runtime = __import__("services.linux_runtime_service", fromlist=["detect_linux_runtime_profile"])
    monkeypatch.setattr(runtime, "detect_linux_runtime_profile", AsyncMock(return_value={"reason": "x"}))
    monkeypatch.setattr(module, "_latest_release_asset", AsyncMock(return_value={"download_url": "https://github.com/a/b/releases/download/v1/p.zip"}))
    prepared = await module._prepare_plugin_execution(
        _Db(), server, SimpleNamespace(id=3), 1, set(), "h" * 64, True, "https://github.com/a/b/releases/download/v2/p.zip"
    )
    assert prepared[4] == {"reason": "x"} and prepared[3][1]["release_tag"] == "v2"
    with pytest.raises(module.PluginPlanError, match="changed"):
        await module._prepare_plugin_execution(_Db(), server, SimpleNamespace(id=3), 1, set(), "x", True, None)

    monkeypatch.setattr(module.inspect, "signature", lambda _progress: (_ for _ in ()).throw(TypeError("signature")))
    called = AsyncMock()
