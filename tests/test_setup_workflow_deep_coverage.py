...
async def test_detect_host_rejects_bad_sudo_and_accepts_passwordless_fallback(monkeypatch):
    ...
    monkeypatch.setattr(setup, "run_sudo_command", AsyncMock(side_effect=[("", "bad", 1), ("", "", 0)]))
    ...
    monkeypatch.setattr(setup, "run_sudo_command", AsyncMock(side_effect=[("", "bad", 1), ("", "still bad", 1)]))
    ...
    with pytest.raises(HTTPException, match="sudo 需要密码"):
...
async def test_install_dependencies_success_optional_and_failure_paths(monkeypatch):
    ...
    monkeypatch.setattr(setup, "run_apt_command_with_retry", AsyncMock(side_effect=lambda *_a, **_k: next(apt_results)))
    conn.results["apt-cache show --no-all-versions p7zip-full >/dev/null 2>&1"] = _result(exit_status=0)
    await setup._install_setup_dependencies(context)
    assert any("可选增强依赖安装完成" in line for line in context.logs)
    assert context.add_command_output.await_count >= 2

    monkeypatch.setattr(setup, "run_apt_command_with_retry", AsyncMock(return_value=("", "update bad", 1)))
    with pytest.raises(HTTPException) as exc_info:
        await setup._install_setup_dependencies(_context(_Conn()))
    assert exc_info.value.status_code == 502

    monkeypatch.setattr(setup, "run_apt_command_with_retry", AsyncMock(side_effect=[("", "", 0), ("", "deps bad", 1)]))
    ...
    monkeypatch.setattr(setup, "run_apt_command_with_retry", AsyncMock(side_effect=[("", "", 0), ("", "", 0)]))
...
async def test_legacy_libssl_directory_user_and_firewall_failures(monkeypatch):
    ...
    monkeypatch.setattr(setup, "_run_setup_command", AsyncMock(side_effect=[("installed\n", "", 0), ("", "", 0)]))
    await setup._install_legacy_libssl(context)
    assert any("安装成功" in line for line in context.logs)
    context = _context(conn, os_version="24.04")
    monkeypatch.setattr(setup, "_run_setup_command", AsyncMock(side_effect=RuntimeError("sftp")))
    await setup._install_legacy_libssl(context)
    assert any("安装过程出错" in line for line in context.logs)

    conn = _Conn({"id cs2server": _result(exit_status=0)})
    context = _context(conn)
    monkeypatch.setattr(setup, "_run_setup_command", AsyncMock(side_effect=[("", "", 1)]))
    with pytest.raises(HTTPException, match="设置 CS2 用户密码失败"):
        await setup._ensure_setup_user(context)

    monkeypatch.setattr(setup, "_run_setup_command", AsyncMock(side_effect=[("", "", 0), ("", "mkdir", 1)]))
    with pytest.raises(HTTPException, match="创建游戏目录失败"):
        await setup._configure_setup_directory(_context(_Conn()))

    firewall = _context(_Conn(), request=_request(open_game_ports=True))
    monkeypatch.setattr(setup, "_run_setup_command", AsyncMock(side_effect=[("Status: active", "", 0), ("", "denied", 1)]))
    await setup._configure_setup_firewall(firewall)
    assert any("开放端口失败" in line for line in firewall.logs)
    inactive = _context(_Conn(), request=_request(open_game_ports=True))
    monkeypatch.setattr(setup, "_run_setup_command", AsyncMock(return_value=("Status: inactive", "", 0)))
    await setup._configure_setup_firewall(inactive)
    assert any("未启用" in line for line in inactive.logs)
...
async def test_persistence_failures_are_non_fatal(monkeypatch):
    ...
    monkeypatch.setattr(setup.redis_manager, "set_initialized_server", AsyncMock(side_effect=RuntimeError("redis")))
    assert await setup._persist_setup_configuration(context, current_user=SimpleNamespace(id=1), db=object()) is None
    assert any("保存到数据库失败" in line for line in context.logs)
    assert any("保存配置失败" in line for line in context.logs)


@pytest.mark.asyncio
async def test_apt_retry_clamps_delay_and_handles_timeout(monkeypatch):
    runner = AsyncMock(side_effect=[asyncio.TimeoutError(), asyncio.TimeoutError(), asyncio.TimeoutError()])
    ...
    result = await setup.run_apt_command_with_retry(
        _Conn(), "apt", description="apt", needs_sudo=False, sudo_password=None,
        add_log=AsyncMock(), attempts=3,
    )
