...
def test_archive_and_filename_helpers(path, expected):
    ...
    assert (
        SSHManager._filename_from_content_disposition("attachment; filename*=bad''a.zip") == "a.zip"
    )
...
def test_response_and_url_validation_helpers():
    ...
    assert (
        SSHManager._filename_from_download_response("garbage", "https://host.example/a.txt")[0]
        is None
    )
    for value in (
    ...
    assert (
        "unsupported"
        in SSHManager._redirect_url_from_response(
...
async def test_resolve_reconnect_and_sftp_retry_paths(monkeypatch):
    ...
    assert (
        ...
        in (await manager._resolve_public_download_address("example.com", "getent"))[1]
    )
    ...
    assert (
        "non-public" in (await manager._resolve_public_download_address("example.com", "getent"))[1]
    )
    ...
    assert await manager._reconnect_current_pooled_connection(_server()) == (
        False,
        None,
        "pool down",
    )
...
async def test_release_proxy_and_github_fetch_fallbacks(monkeypatch):
    assert (
        SSHManager._apply_github_download_proxy("https://example.com/a", "https://proxy")
        == "https://example.com/a"
    )
    assert (
        SSHManager._apply_github_download_proxy("https://github.com/a", "")
        == "https://github.com/a"
    )
    assert (
        SSHManager._apply_github_download_proxy(
    ...
    manager.execute_command = AsyncMock(side_effect=[(False, "", ""), (True, valid, "")])
    assert await manager._fetch_latest_metamod_url() == (True, valid)
    ...
    manager.execute_command = AsyncMock(
        return_value=(True, "https://github.com/a/b/releases/download/v1/a.zip", "")
    )
    ...
    manager.execute_command = AsyncMock(
        side_effect=[
            (False, "", ""),
            (True, "https://other.example/a.zip", ""),
            (True, "v1.2.3", ""),
        ]
    )
