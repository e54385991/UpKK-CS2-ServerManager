...
async def test_plugin_backup_create_empty_and_archive_failure_paths():
    ...
    result = await manager._create_plugin_backup(
        "/csgo", "/backups", "/backups/a.tar.gz", send_progress
    )
...
async def test_plugin_backup_finish_sizes_and_top_level_failures():
    ...
    assert await manager.backup_plugins(_server(), progress.append) == (
        False,
